...
export default function DatasetPage(){
 ...
 try{const r=await fetch(`${root}/${kind}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({schema,table})});if(current!==generation.current)return;if(kind==='insights'&&r.status===503){setAiState('unavailable');return}if(!r.ok)throw Error();const data=await r.json();if(current!==generation.current)return;if(kind==='profile'){setProfile(data);setProfileState('success');setQuality(null);setQualityState('idle');setInsights(null);setAiState('idle')}else if(kind==='quality'){setQuality(data);setQualityState('success');setInsights(null);setAiState('idle')}else{setInsights(data);setAiState('success')}}catch{if(current!==generation.current)return;if(kind==='profile')setProfileState('error');else if(kind==='quality')setQualityState('error');else setAiState('error')}
 }
 ...
 {tab==='Overview'&&<ProfileOverview profile={profile} quality={quality} state={profileState} run={()=>run('profile')}/>}{tab==='Columns'&&<ColumnsPanel profile={profile} state={profileState} run={()=>run('profile')}/>}{tab==='Quality'&&<QualityPanel quality={quality} profile={profile} state={qualityState} run={()=>run('quality')} runProfile={()=>run('profile')} sourceId={id} schema={schema} table={table} onRulesChanged={rulesChanged}/>}{tab==='AI Insights'&&<InsightsPanel available={aiAvailable} state={aiState} insights={insights} run={()=>run('insights')}/>}
 </div></>
}
